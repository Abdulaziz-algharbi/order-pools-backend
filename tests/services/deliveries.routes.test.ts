...
import deliveriesRouter from '../../src/services/deliveries/deliveries.routes';
...
// this breaks if requireRole('ADMIN') is ever removed, reordered after
// the controller, or loosened — not just if requireRole itself misbehaves
// (covered separately in require-role.middleware.test.ts).
function getHandlers(path: string, method: string): Handler[] {
  const stack = (deliveriesRouter as unknown as { stack: RouteLayer[] }).stack;
...
describe('delivery routes: GET operations have no role gate (visibility is checked in the controller)', () => {
  it.each([
    ['GET', '/'],
    ['GET', '/:_id'],
...
describe('delivery routes reject non-ADMIN from write operations', () => {
  it.each([
    ['POST', '/', 'RETAILER'],
    ['POST', '/', 'SUPPLIER'],
    ['PATCH', '/:_id', 'RETAILER'],
...
describe('delivery routes let ADMIN through to the controller on write operations', () => {
  it.each([
    ['POST', '/'],
    ['PATCH', '/:_id'],
    ['DELETE', '/:_id'],
