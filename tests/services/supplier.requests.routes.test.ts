import { Request, Response, NextFunction } from 'express';

import supplierRequestsRouter from '../../src/services/supplier.requests/supplier.requests.routes';

type Handler = (req: Request, res: Response, next: NextFunction) => unknown;

interface RouteStackItem {
  method: string;
  handle: Handler;
}

interface RouteLayer {
  route?: {
    path: string;
    stack: RouteStackItem[];
  };
}

// Walks the actual registered router (not a re-implementation of it), so
// this breaks if requireRole is ever removed, reordered after the
// controller, or loosened — not just if requireRole itself misbehaves
// (covered separately in require-role.middleware.test.ts).
function getHandlers(path: string, method: string): Handler[] {
  const stack = (supplierRequestsRouter as unknown as { stack: RouteLayer[] })
    .stack;
  const layer = stack.find((l) => l.route?.path === path);
  if (!layer?.route) {
    throw new Error(`No route registered for ${method.toUpperCase()} ${path}`);
  }
  const handlers = layer.route.stack
    .filter((s) => s.method === method)
    .map((s) => s.handle);
  if (handlers.length === 0) {
    throw new Error(`No handlers found for ${method.toUpperCase()} ${path}`);
  }
  return handlers;
}

function mockRes() {
  const res: Partial<Response> = {};
  res.status = jest.fn().mockReturnValue(res);
  res.json = jest.fn().mockReturnValue(res);
  res.send = jest.fn().mockReturnValue(res);
  return res as Response;
}

function reqAs(...roles: string[]): Request {
  return {
    meta: { user: { userId: 'caller-1', roles } },
  } as unknown as Request;
}

describe('POST /supplier-requests (RETAILER only)', () => {
  it('lets an authenticated RETAILER pass', () => {
    const handlers = getHandlers('/', 'post');
    const roleGate = handlers[1];
    const req = reqAs('RETAILER');
    const res = mockRes();
    const next = jest.fn();

    roleGate(req, res, next);

    expect(next).toHaveBeenCalled();
    expect(res.status).not.toHaveBeenCalled();
  });

  it.each(['SUPPLIER', 'ADMIN'])('rejects role %s with 403', (role) => {
    const handlers = getHandlers('/', 'post');
    const roleGate = handlers[1];
    const req = reqAs(role);
    const res = mockRes();
    const next = jest.fn();

    roleGate(req, res, next);

    expect(res.status).toHaveBeenCalledWith(403);
    expect(next).not.toHaveBeenCalled();
  });
});

describe('RETAILER/ADMIN-shared supplier-request routes', () => {
  it.each([
    ['GET', '/'],
    ['GET', '/:_id'],
    ['PATCH', '/:_id'],
    ['DELETE', '/:_id'],
  ])('lets an authenticated RETAILER pass %s %s', (method, path) => {
    const handlers = getHandlers(path, method.toLowerCase());
    const roleGate = handlers[1];
    const req = reqAs('RETAILER');
    const res = mockRes();
    const next = jest.fn();

    roleGate(req, res, next);

    expect(next).toHaveBeenCalled();
    expect(res.status).not.toHaveBeenCalled();
  });

  it.each([
    ['GET', '/'],
    ['GET', '/:_id'],
    ['PATCH', '/:_id'],
    ['DELETE', '/:_id'],
  ])('lets an authenticated ADMIN pass %s %s', (method, path) => {
    const handlers = getHandlers(path, method.toLowerCase());
    const roleGate = handlers[1];
    const req = reqAs('ADMIN');
    const res = mockRes();
    const next = jest.fn();

    roleGate(req, res, next);

    expect(next).toHaveBeenCalled();
    expect(res.status).not.toHaveBeenCalled();
  });

  it.each([
    ['GET', '/'],
    ['GET', '/:_id'],
    ['PATCH', '/:_id'],
    ['DELETE', '/:_id'],
  ])('rejects a SUPPLIER-only caller on %s %s with 403', (method, path) => {
    const handlers = getHandlers(path, method.toLowerCase());
    const roleGate = handlers[1];
    const req = reqAs('SUPPLIER');
    const res = mockRes();
    const next = jest.fn();

    roleGate(req, res, next);

    expect(res.status).toHaveBeenCalledWith(403);
    expect(next).not.toHaveBeenCalled();
  });
});

describe('supplier-request routes reject unauthenticated callers', () => {
  it.each([
    ['GET', '/'],
    ['POST', '/'],
    ['GET', '/:_id'],
    ['PATCH', '/:_id'],
    ['DELETE', '/:_id'],
  ])('rejects an unauthenticated caller on %s %s with 401', (method, path) => {
    const handlers = getHandlers(path, method.toLowerCase());
    const roleGate = handlers[1];
    const req = { meta: {} } as Request;
    const res = mockRes();
    const next = jest.fn();

    roleGate(req, res, next);

    expect(res.status).toHaveBeenCalledWith(401);
    expect(next).not.toHaveBeenCalled();
  });
});
