...
import usersRouter from '../../src/services/users/users.routes';
...
describe('users routes reject non-ADMIN from every operation', () => {
  it.each([
    ['GET', '/'],
    ['POST', '/'],
    ['GET', '/:_id'],
    ['PATCH', '/:_id'],
    ['DELETE', '/:_id'],
  ])('rejects an authenticated RETAILER on %s %s with 403', (method, path) => {
  ...
  it.each([
    ...
    'rejects a dual-role RETAILER+SUPPLIER caller on %s %s with 403',
    (method, path) => {
      const handlers = getHandlers(path, method.toLowerCase());
      const roleGate = handlers[1];
      const req = reqAs('RETAILER', 'SUPPLIER');
      const res = mockRes();
      const next = jest.fn();
...
describe('users routes let ADMIN through on every operation', () => {
  it.each([
    ['GET', '/'],
    ['POST', '/'],
    ['GET', '/:_id'],
