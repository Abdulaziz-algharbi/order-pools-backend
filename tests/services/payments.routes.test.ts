...
function getHandlers(path: string, method: string): Handler[] {
  const stack = (paymentsRouter as unknown as { stack: RouteLayer[] }).stack;
...
const ADMIN_RETAILER_ROUTES: [string, string][] = [
  ['GET', '/'],
  ['GET', '/:_id'],
  ['POST', '/:_id/confirm'],
  ['POST', '/:_id/cancel'],
...
const ADMIN_ONLY_ROUTES: [string, string][] = [
  ['POST', '/:_id/retry-refund'],
  ['POST', '/:_id/confirm-refund'],
];

describe('payment routes reject unauthenticated callers everywhere', () => {
  it.each([...ADMIN_RETAILER_ROUTES, ...ADMIN_ONLY_ROUTES])(
...
describe('payment routes shared by ADMIN and RETAILER', () => {
  it.each(ADMIN_RETAILER_ROUTES)(
  ...
  it.each(ADMIN_RETAILER_ROUTES)(
    'lets ADMIN and RETAILER through %s %s',
    (method, path) => {
      const handlers = getHandlers(path, method.toLowerCase());
      const roleGate = handlers[1];

      for (const role of ['ADMIN', 'RETAILER']) {
...
describe('payment routes restricted to ADMIN only', () => {
