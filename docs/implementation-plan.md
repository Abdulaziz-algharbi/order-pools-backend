...
- `Pool.status` now advances automatically through most of its lifecycle: `OPEN -> TARGET_REACHED` on a join that fills the pool (`pool.participants.controller.ts`), `TARGET_REACHED -> DISTRIBUTING` when a delivery is created for it, `DISTRIBUTING -> COMPLETED` (plus each `WAITING` `PoolParticipant -> DELIVERED`) when that delivery's `deliveryStatus` reaches `DELIVERED` (both in `deliveries.controller.ts`), and `OPEN -> CANCELLED` via `PoolController.expirePool`. `Pool.supplierPaymentStatus` is synced to `PAID` when its `SupplierPayout.status` reaches `COMPLETED` (`supplier.payouts.controller.ts`). All of these are best-effort secondary writes (logged, never allowed to fail the primary action) — not wrapped in a transaction; see the transactions note below. `PoolController.update()` still has no state-machine guard of its own, so an admin can still `PATCH` any of these fields to an illegal value directly.
